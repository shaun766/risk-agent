...
/** How much history the engine needs for baselines and behavioural insights. */
...
/**
 * The single translation point between the database and the financial engine.
 *
 * The engine is pure and knows nothing about Prisma; everything it needs is
 * assembled here, in one query batch, so a snapshot is a consistent read rather
 * than a series of interleaved ones.
 */
export async function loadEngineContext(userId: string, asOf: Date = new Date()): Promise<EngineContext> {
  ...
  // Investment outflows are already counted from transactions; holdings supply
